...
def main():
    ...
    teachermodel = timm.create_model('mobilenetv3_large_100', pretrained=True)
    teachermodel.eval()
    teachermodel.cuda()
    # import pdb;pdb.set_trace()
    load_to_MultiModel(model, "pretrainedweight/mobilenetv3_large_100_ra-f55367f5.pth")
    ...
    try:
        for stage in range(4):
            subnet, subnetchoice = model.module.generate_random_subnet(stage)
            print(subnetchoice)
            # setup learning rate schedule and starting epoch
            lr_scheduler, num_epochs = create_scheduler(args, optimizer, stage)
            # print(lr_scheduler.get)
            start_epoch = 0
            ...
            for epoch in range(start_epoch, num_epochs):
                ...
                train_metrics = train_one_epoch(
                    epoch, model, loader_train, optimizer, train_loss_fn, args, subnet, subnetchoice,
                ...
                eval_metrics = validate(model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast,
                                        teacher_model=teachermodel, stage=stage, is_distributed=False, subnet=subnet, subnetchoice=subnetchoice)
                ...
                if model_ema is not None and not args.model_ema_force_cpu:
                    ...
                    ema_eval_metrics = validate(
                        ...
                        log_suffix=' (EMA)', teacher_model=teachermodel, stage=stage, is_distributed=True, subnet=subnet, subnetchoice=subnetchoice)
                ...
                if (epoch + 1) % args.savemodelfreq == 0:
                    name = "Secondstage_" + str(stage) + "epoch" + str(epoch) + ".pth"
...
def train_one_epoch(
        epoch, model, loader, optimizer, loss_fn, args, subnet, subnetchoice,
        lr_scheduler=None, saver=None, output_dir=None, amp_autocast=suppress,
        loss_scaler=None, model_ema=None, mixup_fn=None, stage=None, teacher_model=None):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            #     subnet, subnetchoice = model.module.generate_random_subnet(stage)
            output = model(input, subnet, subnetchoice, stage)
            # print(output.shape, teacher_output.shape)
            if stage < 3:
                loss = torch.norm(teacher_output - output, p=2) / args.batch_size / args.ALPHALOSS
            else:
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix='', teacher_model=None, stage=None,
             is_distributed=False, subnet=None, subnetchoice=None):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
